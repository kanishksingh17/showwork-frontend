...
import { Sparkles, BarChart3, Zap, Palette, GitBranch, Code2, Eye } from "lucide-react";
...
function DisplayCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-lg break-words leading-tight">{description}</p>
...
export default function DisplayCards({ cards }: DisplayCardsProps) {
  const defaultCards: DisplayCardProps[] = [
    ...
    },
    {
      ...
      description: "Share instantly to all platforms",
    ...
    },
    {
      ...
      description: "Personalize colors, fonts, and themes",
      icon: <Palette className="size-4 text-pink-300" />,
      iconClassName: "bg-pink-800",
      titleClassName: "text-pink-500",
      className:
        "[grid-area:stack] translate-x-16 translate-y-20 hover:translate-y-10",
    },
    {
      title: "GitHub Integration",
      description: "Auto-sync projects from your repos",
      icon: <GitBranch className="size-4 text-green-300" />,
      iconClassName: "bg-green-800",
      titleClassName: "text-green-500",
      className:
        "[grid-area:stack] translate-x-24 translate-y-[7.5rem] hover:translate-y-[5rem]",
    },
  ];
  ...
  return (
    <div className="grid [grid-template-areas:'stack'] place-items-center opacity-100 animate-in fade-in-0 duration-700 -translate-x-8">
