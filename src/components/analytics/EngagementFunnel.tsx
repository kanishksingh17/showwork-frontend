...
export const EngagementFunnel: React.FC<EngagementFunnelProps> = ({ funnelData }) => {
                  ...
                  >
                    <div className="flex items-center justify-between text-white">
                      <div>
                        ...
                        <div className="text-sm opacity-90">{step.visitors.toLocaleString()} visitors</div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-semibold">{step.conversionRate.toFixed(1)}%</div>
                        <div className="text-xs opacity-90">{step.avgTime}s avg</div>
                      </div>
                    </div>
                    ...
                    {index > 0 && step.dropOffRate > 0 && (
                      <div className="absolute -right-8 top-1/2 transform -translate-y-1/2">
                        <div className="flex items-center space-x-1 text-red-500">
                          ...
                          <span className="text-sm font-semibold">{step.dropOffRate.toFixed(1)}%</span>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-white/20">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-sm text-slate-600 mb-2">Drop-off Rate</div>
                  <Progress value={funnelData.steps[selectedStep].dropOffRate} className="h-2" />
                  <div className="text-sm text-red-600 mt-1">
                    {funnelData.steps[selectedStep].dropOffRate.toFixed(1)}% drop-off
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-sm text-slate-600 mb-2">Performance Score</div>
                  <div className="text-2xl font-bold text-green-600">
                    {Math.round(100 - funnelData.steps[selectedStep].dropOffRate)}
                  </div>
            ...
            {funnelData.dropOffPoints.map((dropOff, index) => (
              <div key={index} className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                <div className="flex items-center justify-between mb-3">
                  <div className="font-semibold text-slate-800">{dropOff.step}</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <div className="text-sm font-semibold text-slate-700 mb-2">Reasons:</div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
              <div className="font-semibold text-blue-800 mb-2">Medium Impact</div>
