...
export const AdvancedVisualizations: React.FC<AdvancedVisualizationsProps> = ({ data }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-800">Advanced Visualizations</h2>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-white/20">
            <CardHeader>
              <CardTitle className="text-slate-800 flex items-center">
                {React.createElement(visualizations.find(v => v.id === activeVisualization)?.icon || BarChart3, { className: "w-5 h-5 mr-2 text-blue-600" })}
                {visualizations.find(v => v.id === activeVisualization)?.label}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex justify-center">
                {activeVisualization === 'bar' && <InteractiveBarChart data={barData} />}
                {activeVisualization === 'line' && <InteractiveLineChart data={lineData} />}
                {activeVisualization === 'pie' && <InteractivePieChart data={pieData} />}
