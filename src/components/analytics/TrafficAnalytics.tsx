...
export const TrafficAnalytics: React.FC<TrafficAnalyticsProps> = ({ trafficData }) => {
  const [selectedPeriod, setSelectedPeriod] = useState<'daily' | 'weekly' | 'monthly'>('daily');
  const [selectedMetric, setSelectedMetric] = useState<'views' | 'visitors'>('views');

  const getCurrentData = () => {
    switch (selectedPeriod) {
      case 'daily': return trafficData.daily;
      case 'weekly': return trafficData.weekly;
      case 'monthly': return trafficData.monthly;
      default: return trafficData.daily;
    }
  };
          ...
          <CardContent>
            <div className="space-y-4">
              {currentData.slice(-7).map((item, index) => {
                ...
                const isIncreasing = index > 0 && value > getMetricValue(currentData[index - 1]);
                
                return (
                  <div key={index} className="space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-slate-600">{getLabel(item)}</span>
                      <div className="flex items-center space-x-2">
              ...
              {trafficData.sources.map((source, index) => (
                <div key={index} className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-4">
                      <span className="text-slate-600">{source.views} views</span>
