...
export const Template13Inner: React.FC<PortfolioTemplateProps> = ({ userData, projects, sections }) => {
    ...
    const mlopsData = resumeSection?.customData || {};

    const name = String(aboutData.name || userData?.name || 'Nyra');
    ...
    const bio = String(aboutData.bio || userData?.bio || 'I create intelligent solutions with AI, machine learning, and data science to drive the future forward.');
    const headline = String(aboutData.headline || userData?.professionalHeadline || userData?.title || "Empowering Innovation Through AI & Data Science");
    const tagline = String(aboutData.tagline || 'Building tools for developers...');
    const email = aboutData.email || userData?.email || 'contact@nyra-mlops.ai';
            ...
            <nav className="sticky top-0 w-full bg-[#FFD147] py-4 px-8 flex justify-between items-center text-[#1A1A1A] z-50 shadow-sm">
                <div className="flex items-center gap-12">
                    ...
                    <div className="hidden md:flex gap-8 text-sm font-medium">
                        {showHero && <a className="hover:opacity-75 transition-opacity" href="#about">About</a>}
                        {showSkills && <a className="hover:opacity-75 transition-opacity" href="#expertise">Expertise</a>}
                        {showResume && <a className="hover:opacity-75 transition-opacity" href="#experience">Experience</a>}
                        {showContact && <a className="hover:opacity-75 transition-opacity" href="#contact">Contact</a>}
                    </div>
                </div>
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 divide-y md:divide-y-0 md:divide-x divide-white/10">
                                    <div className="flex flex-col gap-2 pt-8 md:pt-0 md:pl-0">
                                        <span className="font-['Space_Grotesk'] text-5xl md:text-6xl font-bold text-white tracking-tighter">{String(mlopsData.metric1Value || "99.9%")}</span>
                                        <span className="text-gray-400 text-lg font-medium">{String(mlopsData.metric1Label || "System Uptime")}</span>
                                        <p className="text-gray-500 text-sm mt-2 max-w-xs">{String(mlopsData.metric1Sublabel || "Ensuring critical ML services are available when needed most.")}</p>
                                    </div>
                                    <div className="flex flex-col gap-2 pt-8 md:pt-0 md:pl-12">
                                        <span className="font-['Space_Grotesk'] text-5xl md:text-6xl font-bold text-white tracking-tighter">{String(mlopsData.metric2Value || "120")}<span className="text-3xl text-[#FFD147] align-baseline ml-1">ms</span></span>
                                        <span className="text-gray-400 text-lg font-medium">{String(mlopsData.metric2Label || "Inference Latency")}</span>
                                        <p className="text-gray-500 text-sm mt-2 max-w-xs">{String(mlopsData.metric2Sublabel || "Optimized serving layers for lightning-fast predictions.")}</p>
                                    </div>
                                    <div className="flex flex-col gap-2 pt-8 md:pt-0 md:pl-12">
                                        <span className="font-['Space_Grotesk'] text-5xl md:text-6xl font-bold text-white tracking-tighter">{String(mlopsData.metric3Value || "20+")}</span>
                                        <span className="text-gray-400 text-lg font-medium">{String(mlopsData.metric3Label || "Deployed Models")}</span>
                                        <p className="text-gray-500 text-sm mt-2 max-w-xs">{String(mlopsData.metric3Sublabel || "Successfully managing a diverse portfolio of production models.")}</p>
                                    </div>
                                </div>
                            </div>
                        </EditableBlock>
