...
export const SREDevOps: React.FC<{ data?: any }> = ({ data }) => {
    const sreData = {
        num: data?.num || "05 / Reliability Engineering (SRE)",
        h2: data?.h2 || "Incidents are\na systems problem.",
        sub: data?.sub || "Not a people problem. Never a people problem.",
        body: data?.body || "I practice SRE as an engineering discipline. Error budgets are contractual commitments. Every on-call rotation has a bounded cognitive load ceiling.",
        timeline: data?.timeline || [
            { dot: <Activity size={14} />, title: "Detection — <2 minutes", desc: "Multi-signal alerting: composite alerts requiring metric + log + trace correlation." },
            { dot: <Target size={14} />, title: "Triage — <5 minutes", desc: "Runbooks are executable. Automated playbook triggers fire first." },
            { dot: <Wrench size={14} />, title: "Remediation — <15 minutes", desc: "Rollback is the default path. Feature flags give instant blast radius control." },
            { dot: <FileText size={14} />, title: "Post-Mortem — Blameless", desc: "5-whys analysis archived in version control. Action items tracked as engineering tickets." }
        ],
        slos: data?.slos || [
            ...
            <div className="section-inner w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                ...
                >
                    <div className="section-header mb-14">
                        <span className="section-num font-mono text-[11px] text-[var(--t07-purple-bright)] tracking-widest uppercase mb-3 block">{sreData.num}</span>
                        <h2 className="section-h2 text-[clamp(32px,4vw,48px)] font-extrabold tracking-tight leading-[1.1] mb-4" dangerouslySetInnerHTML={{ __html: sreData.h2.replace(/\n/g, '<br />') }} />
                        <p className="section-sub font-serif italic text-[clamp(18px,2vw,24px)] text-[var(--t07-purple-edge)] mb-4">{sreData.sub}</p>
                    ...
                    <div className="sre-timeline flex flex-col gap-0">
                        {sreData.timeline.map((item: any, i: number, arr: any[]) => (
                ...
                >
                    ...
                    <div className="slo-grid flex flex-col gap-4">
                        {sreData.slos.map((slo: any, i: number) => (
