...
export const PipelineDevOps: React.FC<{ data?: any }> = ({ data }) => {
    const pipelineData = {
        num: data?.num || "02 / CI/CD & Automation",
        h2: data?.h2 || "Deploy in minutes.\nRollback in seconds.",
        sub: data?.sub || "Pipeline architecture that ships trust, not just code.",
        body: data?.body || "I architect CI/CD systems as first-class infrastructure: GitOps-native, policy-enforced, and observable end-to-end.",
        techTags: data?.techTags || ["ArgoCD", "GitHub Actions", "Tekton", "Helm", "Kustomize", "OPA / Gatekeeper", "Sigstore", "SBOM"],
        stages: data?.stages || [
    ...
    return (
        <section id="cicd">
            <div className="section-inner w-full max-w-5xl">
                ...
                >
                    <span className="section-num font-mono text-[11px] text-[var(--t07-purple-bright)] tracking-widest uppercase mb-3 block">{pipelineData.num}</span>
                    <h2 className="section-h2 text-[clamp(32px,4vw,48px)] font-extrabold tracking-tight leading-[1.1] mb-4" dangerouslySetInnerHTML={{ __html: pipelineData.h2.replace(/\n/g, '<br />') }} />
                    <p className="section-sub font-serif italic text-[clamp(18px,2vw,24px)] text-[var(--t07-purple-edge)] mb-4">{pipelineData.sub}</p>
                    <p className="section-body text-[14px] leading-[1.75] text-[var(--t07-text-secondary)] max-w-[680px]">
                        {pipelineData.body}
                    </p>
                    <div className="mt-8 flex flex-wrap gap-2">
                        {pipelineData.techTags.map((tag: string) => (
                ...
                >
                    ...
                    <div className="cicd-metrics grid grid-cols-2 lg:grid-cols-4 gap-4 mt-10">
                        {pipelineData.metrics.map((metric: any, i: number) => (
