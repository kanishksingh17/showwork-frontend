...
const QuickAddProject: React.FC<QuickAddProjectProps> = ({
  ...
  const handleAutoSave = async () => {
    ...
    try {
      ...
      if (
        previousProjectData &&
        JSON.stringify(previousProjectData) !== JSON.stringify(projectData)
      ) {
      ...
      const existingDrafts = JSON.parse(
        localStorage.getItem("project-drafts") || "[]",
      );
      const updatedDrafts = existingDrafts.filter(
        (draft: any) => draft.id !== draftData.id,
      );
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div
        className={`grid gap-6 ${showVersionHistory ? "grid-cols-1" : "grid-cols-1 lg:grid-cols-2"}`}
      >
        {/* Form Column */}
                ...
                <GitHubScraper
                  ...
                  onDataScraped={(data) => {
                    setProjectData((prev) => ({
                      ...
                      techTags: [
                        ...prev.techTags,
                        ...Object.keys(data.languages).slice(0, 3),
                      ],
                      category: data.topics.includes("web")
                        ? "Web Development"
                        : data.topics.includes("mobile")
                          ? "Mobile App"
                          : data.topics.includes("desktop")
                            ? "Desktop App"
                            : data.topics.includes("data")
                              ? "Data Science"
                              : "Web Development",
                    }));
                  }}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("primaryTag", e.target.value)
                    }
                    onBlur={handleFieldBlur}
                    className="pl-10"
                  />
